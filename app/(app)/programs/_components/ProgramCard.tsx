...
export function ProgramCard({ prog, index }: ProgramCardProps) {
  ...
  const badgeClasses =
    prog.id === "bsc-data-science"
      ? "bg-primary text-primary-foreground"
      : prog.id === "pgd-data-analytics"
  ...
  return (
    ...
    >
      ...
      <div
        className={`lg:col-span-5 relative aspect-16/10 lg:aspect-auto min-h-60 overflow-hidden bg-muted ${index % 2 === 1 ? "lg:order-2" : ""}`}
      >
        <ImageWithFallback
          src={prog.image}
          alt={prog.title}
      ...
      {/* Content */}
      <div
        className={`lg:col-span-7 p-8 md:p-10 flex flex-col justify-between ${index % 2 === 1 ? "lg:order-1" : ""} bg-background`}
      >
        <div>
          {/* Badge + title */}
          ...
          <h2
            className="text-2xl md:text-3xl mb-1 leading-tight font-bold"
            style={{ fontFamily: "var(--font-serif)" }}
          >
            {prog.title}
          </h2>
          <p className={`${textClass} font-medium mb-4 text-sm`}>
            {prog.subtitle}
          </p>
