...
export default async function NewsArticleRoute({ params }: PageProps) {
  ...
  return (
    <div>
      ...
      {/* Article Header */}
      <article className="max-w-[1200px] mx-auto px-4 md:px-6 lg:px-8 bg-background text-foreground">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-12 py-8">
          ...
          <div className="lg:col-span-8">
            <div className="max-w-none text-foreground leading-relaxed space-y-6 text-lg" style={{ fontFamily: 'var(--font-sans)' }}>
              {article.content.split('\n\n').map((para, i) => {
                const cleanPara = para.trim();
                if (cleanPara.startsWith('<h3>')) {
                  const headingText = cleanPara.replace('<h3>', '').replace('</h3>', '');
                  return (
                    <h3 key={i} className="text-2xl font-semibold mt-8 mb-4 font-serif" style={{ fontFamily: 'var(--font-serif)' }}>
                      {headingText}
                    </h3>
                  );
                }
              ...
              <div>
                ...
                <div className="space-y-4">
                  {relatedArticles.map((related) => (
                    ...
                    >
                      <div className="relative aspect-[16/9] overflow-hidden mb-3 bg-muted">
                        <ImageWithFallback
                          src={related.image}
                          alt={related.title}
                      ...
                      <h4 className="font-medium leading-snug group-hover:text-primary transition-colors text-[16px]" style={{ fontFamily: 'var(--font-serif)' }}>
                        {related.title}
                      </h4>
