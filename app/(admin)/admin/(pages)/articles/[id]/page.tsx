...
export default function ArticleDetailRoute({ params }: Props) {
  const { id } = use(params);
  const router = useRouter();
  const { currentUser } = useMockAuth();
  
  const [submission, setSubmissionState] = useState(() => getSubmissionById(id));
  ...
  // Editorial check: is the editor currently allowed to edit?
  const canEditorEdit =
  ...
  const handleSendToReview = () => {
    // Optionally trigger save beforehand
    const tags = tagsInput
      .split(",")
      .map((t) => t.trim())
      .filter((t) => t.length > 0);
    ...
    transitionStatus("in_review", "Editor completed drafts edits and submitted for peer review.");
  };
  ...
  const handleAdminOverride = (newStatus: SubmissionStatus) => {
    transitionStatus(
      newStatus,
      `Admin override: Forcefully shifted status to ${newStatus}.`
    );
  };
  ...
  return (
    <div className="space-y-8 max-w-5xl mx-auto">
      ...
      <div className="bg-card border border-border p-6 rounded shadow-sm flex flex-col md:flex-row justify-between items-start md:items-center gap-6">
        ...
        {/* Workflow actions (Flat render depending on role & status constraints) */}
        <div className="flex flex-wrap gap-2 w-full md:w-auto border-t md:border-t-0 pt-4 md:pt-0">
          ...
          {currentUser.role === "editor" && (
            <>
              {["submitted", "changes_requested"].includes(submission.status) && (
                <Button onClick={handleStartEditing} className="bg-primary text-primary-foreground">
                  <Play className="w-4 h-4 mr-1.5" />
                  Start Editing
                </Button>
              )}
              {submission.status === "in_editing" && (
                <Button onClick={handleSendToReview} className="bg-amber-600 hover:bg-amber-700 text-white">
          ...
          {currentUser.role === "reviewer" && submission.status === "in_review" && (
            <div className="space-y-4 w-full">
              ...
              <div className="flex gap-2">
                <Button onClick={handleReviewApprove} className="bg-emerald-600 hover:bg-emerald-700 text-white flex-1 sm:flex-initial">
                  <Check className="w-4 h-4 mr-1.5" />
                  Approve Publication
                </Button>
                <Button onClick={handleReviewReject} variant="destructive" className="flex-1 sm:flex-initial">
          ...
          {currentUser.role === "admin" && (
            <div className="bg-rose-50/50 dark:bg-rose-950/10 border border-rose-200 dark:border-rose-900 p-3 rounded flex items-center gap-4 w-full">
              ...
              <Select onValueChange={(val) => handleAdminOverride(val as SubmissionStatus)} value={submission.status}>
          ...
          {canEditorEdit ? (
            ...
            <form onSubmit={handleEditorSave} className="bg-card border border-border p-6 rounded shadow-sm space-y-6">
              ...
              <div className="flex justify-between items-center pt-4 border-t border-border">
                ...
                <Button type="submit" className="bg-primary text-primary-foreground rounded-none">
                  Save Changes
                </Button>
              </div>
            </form>
            ...
            <div className="bg-card border border-border p-6 rounded shadow-sm space-y-6 text-foreground">
              ...
              {/* Cover image preview if URL exists */}
              {submission.image && (
                <div>
              ...
              <div className="flex flex-wrap gap-2 pt-2">
                {submission.tags.map((t) => (
                  <span
                    key={t}
                    className="text-xs px-2.5 py-1 bg-muted text-muted-foreground rounded-sm font-medium"
                  >
                    #{t}
                  </span>
                ))}
          ...
          <div className="bg-card border border-border p-5 rounded shadow-sm">
            ...
            <div className="space-y-6 relative before:absolute before:left-4 before:top-2 before:bottom-2 before:w-0.5 before:bg-border">
              {submission.history.map((hist, idx) => {
                const badge = statusBadgeConfig[hist.status] || {
                  label: hist.status,
                  bg: "bg-muted",
