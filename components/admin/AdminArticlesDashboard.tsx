...
export function AdminArticlesDashboard({ submissions, currentUser }: Props) {
  const router = useRouter();
  const [filterTab, setFilterTab] = useState<
    "action_items" | "all" | "published" | "drafts"
  >("action_items");

  // Tab filter — the DB/RLS already scoped `submissions` to what this role
  // can see (reviewer -> in_review only, editor/admin -> everything).
  // This just further narrows the view for editor/admin tabs.
        ...
        <div>
          ...
          <p className="mt-1.5 text-sm text-muted-foreground">
            {currentUser.role === "editor" &&
              "Manage workflow submissions, refine drafts, and publish approved articles."}
            {currentUser.role === "reviewer" &&
              "Review drafts, approve them, or request revisions from editors."}
            {currentUser.role === "admin" &&
              "Administrative override panel with full visibility across all submissions."}
          </p>
        </div>
      </div>
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4 border border-border bg-card p-4">
        {[
          ...
          },
          {
            ...
            count: submissions.filter((s) => s.status === "in_review").length,
          ...
          <div
            key={i}
            className="text-center py-2 border-r border-border last:border-0"
          >
            <div className={cn("text-3xl font-bold font-serif", stat.color)}>
              {stat.count}
            </div>
            <div className="text-[10px] text-muted-foreground uppercase font-semibold tracking-wider mt-1">
              {stat.label}
            </div>
          </div>
        ))}
